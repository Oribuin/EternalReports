...
class CmdResolve(private val plugin: EternalReports, command: OriCommand) : SubCommand(command, "resolve") {
    ...
    private fun resolvedFormatted(resolved: Boolean): String? {
        return if (resolved) {
            plugin.getManager(MessageManager::class).messageConfig.getString("resolve-formatting.is-resolved")?.let { HexUtils.colorify(it) }
        } else {
            plugin.getManager(MessageManager::class).messageConfig.getString("resolve-formatting.isnt-resolved")?.let { HexUtils.colorify(it) }
        }
    }
}